...
var navigator = {};
var document = { createElement: function() { return { setAttribute: function() {} } } };
var window = {};
...
//  var cmp = CodeMirror.cmpPos = function(a, b) { return a.line - b.line || a.ch - b.ch; };
  var cmp = function(a, b) { return a.line - b.line || a.ch - b.ch; };
  CodeMirror.cmp = cmp;
  ...
  CodeMirror.selectInput = selectInput;
  ...
  CodeMirror.bidiOrdering = bidiOrdering;

  // THE END

  CodeMirror.version = "4.0.4";

  var mySelf = String(FOO).slice('function'.length);
  //print(mySelf);
  var regexp = /function\s+(\S)+\(/g;
  var matches = mySelf.match(regexp);
  for (var i = 0; i < matches.length; i++) {
    var name = matches[i].slice('function '.length);
    name = name.slice(0, name.length - 1);
    if (name == 'CodeMirror') continue;
    try {
      var x = eval(name);
      CodeMirror[name] = x;
    } catch (e) {
...
function transformSource(src) {
  ...
  src = src.replace(/instanceof/g, "is");
  src = src.replace(/===/g, "==");
  src = src.replace(/!==/g, "!=");
  src = src.replace(/\+new Date/g, "currentTimeInMs()")
  src = src.replace(/([^"])function/g, "$1")

  //typeof text == "string"
  src = src.replace(/typeof (.+) == ("[a-zA-Z]+")/g, "typeOfReplacement($1, $2)")
  src = src.replace(/(\/.+\/[gim]?)/g, 'new Refexp("$1")')
...
function printConstructor(name, fn) {
  ...
  for (var k in fn.prototype) {
    ...
    printMethod(k, fn.prototype[k]);
  }
  print('}');
}

for (var k in this.CodeMirror) {
  var fn = this.CodeMirror[k];
  if (typeof fn != 'function') continue;
  var firstLetter = k[0];
  if (firstLetter == '_') firstLetter = k[1];
  if (firstLetter.toUpperCase() == firstLetter) {
    printConstructor(k, fn);
  } else {
    printMethod(k, fn);
  }
  print('');
